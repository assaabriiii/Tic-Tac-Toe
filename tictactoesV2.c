...
int main()
{
    ...
    while (status == 1)
    {
        ...
        printf("Do you want to play again ? (Send number 1 to continue)\n"); // Check if player want to play again or not if #yes 
        //the we call our reset function which reset all the arrays and summon the default numbers and if the answer was #no then exit the game will automatically runs
        scanf("%d",&status);
        if (status == 1)
        {
            mainStatus = -1;
...
void reset() // our reset function which run the all 9 houses and reset them by using #change function 
{
    change(mainboard);
    change(part1);
    change(part2);
...
void change(char board[10]) // here the change function will reset each of the 9 houses to the default numbers 
{
    board[0] = '0';
    board[1] = '1';
    board[2] = '2';
